...
function UI:CreateSettingsDisplay()
	...
	local idleSlider = AceGUI:Create("Slider")
	idleSlider:SetValue(Addon.db.settings.minimap.idleThresholdSeconds)
	idleSlider:SetSliderValues(1, 300, 5)
	idleSlider:SetWidth(300)
	idleSlider:SetLabel(L["Idle threshold for gold per hour calculation"])
	openerCheckbox:SetCallback("OnMouseUp", function(_, event, value) UI:SendMessage(Events.Minimap.IdleThreshold, value) end)
	checkboxContainer:AddChild(idleSlider)
...
function UI:CreateStatsDisplay()
	...
	mapTotalLabel:SetFontObject(GameFontNormalSmall)
	...
	mapAverageLabel:SetFontObject(GameFontNormalSmall)
	...
	mapMarksLabel:SetFontObject(GameFontNormalSmall)
